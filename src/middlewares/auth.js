  ...
  try {
    ...
    if (req.userData.rol === 2) {
      ...
      const resultDoctor = await db.query(
        "SELECT id_doctor FROM doctores WHERE id_usuario = $1 AND estado_verificacion = $2",
        [req.userData.id, "Aprobado"]
      );
      ...
      if (resultDoctor.rows.length > 0) {
        ...
        // O verificar si el paciente está asignado a este doctor mediante la tabla doctor_paciente
    ...
    if (req.userData.rol === 3) {
      ...
      if (
        ...
        resultPaciente.rows[0].id_paciente === idPacienteRecurso
      ) {
        return next();
      }
    }
