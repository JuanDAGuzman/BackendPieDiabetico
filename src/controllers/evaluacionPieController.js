...
exports.createEvaluacion = async (req, res, next) => {
  try {
    ...
    const evaluacionCompleta = await EvaluacionPie.findById(nuevaEvaluacion.id_evaluacion);
...
exports.deleteEvaluacion = async (req, res, next) => {
  try {
    ...
    if (!deletedEvaluacion) {
      return res.status(500).json({ message: 'Error al eliminar la evaluación' });
    }
...
exports.getEvaluacionesByPaciente = async (req, res, next) => {
  try {
    ...
    const evaluaciones = await EvaluacionPie.getEvaluacionesByPaciente(id_paciente);
